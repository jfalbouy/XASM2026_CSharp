...
int calc_hash(unsigned char *name)
{
   int sum = 0;
...
int calc_hash2(unsigned char *name)
{
   int sum = 0;
   unsigned char *p = name;
   while ( *p!='\0' ) {   sum += *(p++);  }
...
label_typ *add_lblhash(label_typ **hash, unsigned char *id_name, sc_reg id_value)
{
   label_typ *p,*l;
   int h;
   h=calc_hash(id_name);
...
label_typ *chk_lblhash(label_typ **hash, unsigned char *id_name, sc_reg *id_value)
{
   label_typ *p;
   int h;
...
int trace_symbol_hash(label_typ **hash)
{
   ...
   for (h=0;h<maxhash;h++) {
      ...
      while (1) {
         ...
         if (p->child!=NIL) {
            l_stack[l_stack_p-1] = (label_typ *)p;  l_stack_p++;
...
macro_typ *check_macro(unsigned char *name)
{
   macro_typ *m = macros;
   while (1) {
      if (m == NULL) { break; }
