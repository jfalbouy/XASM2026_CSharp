...
int main(int argc, char *argv[])
{
   ...
   err_on = FALSE;
   ihex_on = FALSE;      srec_on = FALSE;      map_on = FALSE;
   dep_on = FALSE;       verbose_on = FALSE;   size_report_on = FALSE;
   basic_uu_on = FALSE;   hxd_on = FALSE;
   title();
   if (argc < 2) {  usage();  }
   obj_typ = '\0';
   pp = isorg(argv[1]);
   ...
   default_ext(ihex_name, source_name, "hex");
   default_ext(srec_name, source_name, "s19");
   default_ext(map_name, source_name, "map");
   default_ext(dep_name, source_name, "d");
   default_ext(basic_uu_name, source_name, "uu");
   default_ext(hxd_name, source_name, "txt");
   for (i = 2; i <= argc - 1; i++) {
      strcpy(inbuf,argv[i]);
      if (top(inbuf) == '-') {
         switch (c2upper(inbuf[1])) {
            case 'L':
               listing_on = TRUE;
               parse_option_filename(&i, argc, argv, listing_name);
            ...
            case 'O':
               object_on = TRUE;
               parse_option_filename(&i, argc, argv, object_name);
            ...
            case 'I':
               ihex_on = TRUE;
               parse_option_filename(&i, argc, argv, ihex_name);
               break;
            case 'M':
               srec_on = TRUE;
               parse_option_filename(&i, argc, argv, srec_name);
               break;
            case 'D':
               dep_on = TRUE;
               parse_option_filename(&i, argc, argv, dep_name);
               break;
            case 'B':
               basic_uu_on = TRUE;
               parse_option_filename(&i, argc, argv, basic_uu_name);
               break;
            case 'X':
               hxd_on = TRUE;
               parse_option_filename(&i, argc, argv, hxd_name);
               break;
            case 'S':  symbol_on  = TRUE;                   break;
            case 'C':  count_on   = TRUE;                   break;
            case 'T':
               if (strlen(inbuf) > 2) { obj_typ = c2upper(inbuf[2]); }
               else if (i + 1 < argc) { i++; obj_typ = c2upper(argv[i][0]); }
               break;
            ...
            case 'P':
               map_on = TRUE;
               parse_option_filename(&i, argc, argv, map_name);
               break;
            case 'R':  size_report_on = TRUE;               break;
   ...
   do {
      ...
      do {
         ...
         do {
            if ( (current_ifdef_stat == 0)||
               ...
               if ((asmtext[0]==' ')||(asmtext[0]=='\t')) {
                  ...
                  if (strncmp(&asmtext[i],"ifndef",6)==0) { c=1; }
                  if (strncmp(&asmtext[i],"IFEQ"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"ifeq"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"IFNE"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"ifne"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"IFGT"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"ifgt"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"IFLT"  ,4)==0) { c=1; }
                  if (strncmp(&asmtext[i],"iflt"  ,4)==0) { c=1; }
         ...
         strcpy(listing_line,asmtext); new_line = TRUE;
         stradd(asmtext,0x1a);
         label_exist = FALSE;
         pp = 1;
         if ((err == 0)&&(top(asmtext) != ';')) {
            if ((top(asmtext) != ' ')&&(top(asmtext) != '\t')) {
               ...
               if ((asmtext[pp-1]!=':')||(len(x_label)>16)||(isnul(x_label))) {
                  ...
                  if (pass_sw == 1) {
                     err = find_label(x_label,&x,(label_typ ***)&next_pnt);
                     if (err == 11) {
                     ...
                     i = find_label(x_label,&x,(label_typ ***)&equ_pnt); equ_pnt = equ_pnt2;
                  }
                  top_of_line = FALSE;
               }
            }
      ...
      printf("    %d line(s)\n",lines);
      if (pass_sw == 3) { section_finish(lc); }
      if (fatal_err == FALSE) {
         switch (pass_sw) {
            case 1:
