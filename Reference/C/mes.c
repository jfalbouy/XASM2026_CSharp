...
int usage(void)
{
   ...
   printf(" -I[filename] : Intel HEX output (.hex)\n");
   printf(" -M[filename] : Motorola S-Record output (.s19)\n");
   printf(" -P[filename] : MAP output (symbols and sections)\n");
   printf(" -D[filename] : Dependency file output (.d)\n");
   printf(" -B[filename] : BASIC uuencode self-decoder output (.uu)\n");
   printf(" -X[filename] : HxD-style text dump output (.txt)\n");
   printf(" -V           : Verbose errors with column hint\n");
   printf(" -R           : Section size report\n");
   printf("\n");
   printf("   ex. XASM sourcefile -L -O -S -TZ -I -M -P -D -B -X -R -W\n");
   exit(1);
}
...
int err_handle(void)
{
   ...
   if (err != 0) {
      ...
      if ((warning == FALSE)||(warning_on == TRUE)) {
         if (verbose_on == TRUE) {
            snprintf(errtext, sizeof(errtext), "%s\t%d\tcol %d\t%s",
                        current_file->name,current_file->lines,pp,inbuf);
         } else {
            snprintf(errtext, sizeof(errtext), "%s\t%d\t%s",
