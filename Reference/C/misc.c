...
int r_to_i32(sc_reg x, int *ih, int *il)
{
...
int i32_to_r(int ih, int il, sc_reg *x)
{
...
int replace(unsigned char *pat, int pos, int l)
{
...
int strmatch(unsigned char *txt, unsigned char *pat)
{
   int i,j,m,n,f[256];
...
int compf(unsigned char *pat, int m, int *f)
{
   int i,j;
   i=0; j=1; f[1]=0;
   while (j<m) {
