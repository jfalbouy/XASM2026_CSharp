   ...
   switch (opnum) {
         ...
         if (asmtext[pp-1] != ',') {
            ...
            if (err == 0) {
               if (label_exist == TRUE) {
                  ...
                  if ((struct_on == TRUE)&&(undef_flag == FALSE)&&(undef_flag2 == FALSE)) {
                     if (x + 1 > struct_size) { struct_size = x + 1; }
                  }
               } else {
                  err = 15;
               }
      ...
      case 66:                       /* LOCAL */
         ...
         if (label_exist == FALSE) {
            ...
            if (pass_sw == 1) {
               ...
               err = find_label(x_label,&x,(label_typ ***)&equ_pnt); equ_pnt=equ_pnt2;
            }
         }
         l_stack_p++;
         if (l_stack_p<=16) {
            if (pass_sw == 1)   {
               l_stack[l_stack_p-1] = (label_typ *)equ_pnt; equ_pnt->child=new_lblhash();
               if (equ_pnt->child==NULL) {
                  printf("Cannot create local block(%s)\n",equ_pnt->name);
                  exit(1);
               }
            } else {
               l_stack[l_stack_p-1] = (label_typ *)equ_pnt;
            }
         } else {
            err = 16;
         }
      ...
      case 87:   i = 60;             /* MACRO  */
         ...
         while (1) {
            ...
            if (pass_sw == 1) {
               l=len(listing_line)+1;
               mac->define[l_no]=(unsigned char *)malloc(l*sizeof(char));
               strcpy(mac->define[l_no],listing_line);
            }
      ...
      case 94:   i = 60;             /* ENDIF  */
         ...
         break;
      case 95:   i = 60;             /* REPEAT */
         err = emit_repeat_block();
         break;
      case 96:   i = 60;             /* ENDR */
         err = 7;
         break;
      case 97:   i = 60;             /* IFEQ */
         err = enter_numeric_if(0);
         break;
      case 98:   i = 60;             /* IFNE */
         err = enter_numeric_if(1);
         break;
      case 99:   i = 60;             /* IFGT */
         err = enter_numeric_if(2);
         break;
      case 100:  i = 60;             /* IFLT */
         err = enter_numeric_if(3);
         break;
      case 101:  i = 60;             /* STRUCT */
         if (label_exist == FALSE) { err = 15; break; }
         struct_on = TRUE;
         struct_size = 0;
         stricpy(struct_name, x_label, 31);
         break;
      case 102:  i = 60;             /* ENDS */
         if (struct_on == FALSE) { err = 7; break; }
         err = close_struct();
         break;
      case 103:  i = 60;             /* SECTION */
         get_operand(&pp,inbuf); slim(inbuf);
         if (isnul(inbuf)) { err = 21; }
         else if (pass_sw == 3) { section_start(inbuf); }
         break;
      default:
         break;
   }
