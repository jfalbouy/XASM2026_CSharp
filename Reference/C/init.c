...
int set_hash(void)
{
   ...
   add_hash( "ENDIF"    , 94 );
   add_hash( "REPEAT"   , 95 );   add_hash( "ENDR"     , 96 );
   add_hash( "IFEQ"     , 97 );   add_hash( "IFNE"     , 98 );
   add_hash( "IFGT"     , 99 );   add_hash( "IFLT"     , 100 );
   add_hash( "STRUCT"   , 101 );  add_hash( "ENDS"     , 102 );
   add_hash( "SECTION"  , 103 );
   return 0;
}
