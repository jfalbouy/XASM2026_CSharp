...
(undefined-symbol detection, cyclic-include detection). `PrebyteFamiliesTests` assembles every
instruction with an internal-RAM operand (2520 cases: `pre_on`/`pre_off` × `(n)`, `BP+n`, `PX+n`,
`PY+n`, `BP+PX`, `BP+PY`) and compares it to bytes produced by the C engine
(`tools/gen_prebyte_families.py` → `tests/prebyte_families.expected.txt`), refusals included. The
trap it guards: `InternalRamOffset` **emits the PRE byte as a side effect**, so it must be called
*before* the opcode, and two internal operands need **one** combined `EmitPrebyte(first, second)`
(see `EmitMemoryPointerTransfer` for the `F0`–`FB` family). `.github/workflows/ci.yml` runs build
...
| `rel` (prefix) | A62 relocation: `[label:] rel <instruction>` assembles the instruction and records its address field as a relocatable **site**. At the end of assembly the sites are encoded into an **A62/Kon relocation table** appended after the code — reproducing the A62 compiler, not hand-coding the table. Table format (reverse-engineered, byte-exact vs `PLINKC.OBJ`): deltas between successive address-field offsets (first delta from the org), bit `080h` = 3-byte width (`lmn`: `JPF`/`CALLF`, `MV r3,imm20`, every `[lmn]` form, `MVP (n),lmn`, `DP`), absent = 2-byte (`mn`: `JP`/`CALL`/`JPZ`/`JPNZ`/`JPC`/`JPNC`, `DW`), `07Eh` = long delta (2-byte LE follows), `0FFh` = end. The field's position and width are **noted by the encoder when it emits the address** (`NoteAddressField`, called from `Emit24`, `EmitAbsoluteJump` and `EmitData` for DW/DP) — never inferred from the end of the instruction, which was wrong for conditional near jumps, for forms where a byte follows the address (`CMP [lmn],n`, `MV [lmn],(n)`…) and for `DW`. A `rel` line must carry **exactly one** absolute address, otherwise it is a fatal error (a bogus site could encode a negative delta as `0FFh`, the terminator). Implemented in `NativeAssembler` (`_relocSites`/`_relAddressFields`/`AppendRelocTable`/`EncodeRelocTable`); emitted only when `rel` sites exist, so a source without `rel` is byte-identical (goldens safe). `RelocationSitesTests` checks the table against the bytes that actually move when the origin changes |
