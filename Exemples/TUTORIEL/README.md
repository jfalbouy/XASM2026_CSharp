...
| `10_cmp_test_formes.asm` | **CMP et TEST : formes valides et formes refusées.** Toutes les formes réelles (`CMP A,n` `60`, `(m),n` `61`, `[lmn],n` `62`, `(n),A` `63`, `(m),(n)` `B7` ; `TEST` `64`–`67`) face aux familles qui ont un `A,(n)` (`42`…`6F`). Montre comment réécrire `cmp a,(n)` — qui **n'existe pas** — en `cmp (n),a` **en inversant la condition** (table `jrc`/`jrnc`). Les formes refusées (`cmp a,(n)`, `test a,(n)`, `test (m),(n)`) sont dans un bloc `ifne demo_refus` : le passer à 1 → `Undefined instruction`. Objet **identique** avec le moteur C `xasm2026-1-2`. | *Syntaxe assembleur → modes d'adressage* |
