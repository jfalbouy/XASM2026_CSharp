...
;----------------------------------------------------------------------
;		Differentes definitions numeriques
;----------------------------------------------------------------------
fcs:			equ	$fffe4
iocs:			equ	$fffe8
iocs_fook:		equ	$bfca2
...
;	Si un bloc de memoire existe
	...
	mv	y,[iroot]			;	Debut de la liste d'en-tete IOCS d'origine
	mv	[ihead],y

;	Definir le numero de peripherique
	pre	$32
	mv	(cx),10-1			;	Numero de peripherique planifie 10
	{
		pre	$32
		inc	(cx)
		mv	il,1
	...
	pre	$32
	mv	[devno],(cx)		;	Definir le numero de peripherique
...
msend2:
	...
	mv	a,0
	pre	$32
	mv	[(iwork)+$3a],a	;	Detachez RAMFILE
	popu	i
msend:	call	putm2
	pre	$32
	add	($ec),%
...
devno:		db	0			;Numero d'appareil (variable)
		...
		dp	devmain		;Adresse du corps du pilote de peripherique rel
